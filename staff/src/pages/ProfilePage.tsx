  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <div className="rounded-2xl bg-gradient-to-r from-indigo-600 via-indigo-500 to-violet-500 p-6 text-white shadow-lg">
        ...
        <p className="mt-1 text-sm text-indigo-100">Personal details, account state, and security controls.</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center gap-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-800">{employee.firstName} {employee.lastName}</h2>
            <p className="text-sm text-slate-500">{employee.staffId}</p>
          </div>
        </div>

        <div className="divide-y text-sm">
          <div className="flex justify-between py-3"><span className="text-slate-500">Email</span><span className="font-medium">{employee.email || '—'}</span></div>
          <div className="flex justify-between py-3"><span className="text-slate-500">Phone</span><span className="font-medium">{employee.phone || '—'}</span></div>
        ...
        {changingPin && (
          <form onSubmit={changePin} className="mt-4 space-y-3">
            <input type="password" maxLength={4} placeholder="Current PIN" value={pinForm.currentPin}
              ...
              className="w-full rounded-xl border border-slate-300 p-2 text-sm" required />
            ...
            <button type="submit" disabled={saving}
              className="w-full rounded-xl bg-indigo-600 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50">
      ...
      <button onClick={logout} className="w-full rounded-xl border border-red-200 py-2 text-sm font-medium text-red-600 hover:bg-red-50">
        Sign Out
      </button>
    </div>
  );
